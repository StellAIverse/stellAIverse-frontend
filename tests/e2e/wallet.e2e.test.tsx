...
const MOCK_BALANCES = [
  ...
  { asset: 'USDC', balance: '50.0000000', assetCode: 'USDC', assetIssuer: 'GBBD...' },
];
...
describe('E2E: Wallet Connection Flow', () => {
  ...
  test('complete wallet connection flow: connect and display wallet', async () => {
    ...
    expect(localStorage.getItem('stellar_wallet_address')).toBe(MOCK_WALLET_ADDRESS);
  });
  ...
  test('complete wallet flow: network switching', async () => {
    ...
    const networkBtn = screen.getByText(/Mainnet|Testnet|Futurenet/);
  ...
  test('complete wallet flow: disconnect wallet', async () => {
    ...
    const walletBtn = screen.getByText(/G[A-Z0-9]{3,}/);
    if (walletBtn) {
  ...
  test('E2E: wallet persistence across page refreshes', async () => {
    ...
    expect(screen.getByText(/Welcome.*GAAA/)).toBeInTheDocument();
  });
...
describe('E2E: Wallet Address Display', () => {
  test('wallet address is displayed in truncated format', async () => {
    ...
    await waitFor(() => {
      const addressDisplay = screen.getByText(/GAAA.*V6ST/);
...
describe('E2E: Copy to Clipboard', () => {
  test('copy address to clipboard functionality', async () => {
    ...
    const walletBtn = screen.getByText(/GAAA.*V6ST/);
    await user.click(walletBtn);
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(MOCK_WALLET_ADDRESS);
  });
});
